...
if __name__ == '__main__':
  ...
  if cache_file_name.exists():
    ...
    all_classes, all_indexes, all_images = prepare_data()
    torch.save({'all_classes':all_classes,
                'all_indexes':all_indexes,
                'all_images' :all_images}, cache_file_name, pickle_protocol=4)
    print ('save cache file into {:}'.format( cache_file_name ))
  ...
  if cache_file_name.exists():
    ...
    leaf_classes = sorted( all_classes.copy() )
    #leaf_classes = [leaf for leaf in all_classes if parser.get_max_depth(leaf)==7]
    coarse_classes = [] 
    for leaf_class in leaf_classes:
      ancs = parser.get_ancestors(leaf_class)
      for a in ancs:
        coarse_classes.append(a)
    coarse_classes = sorted( list(set(coarse_classes)) )
    for xleaf in leaf_classes:
      for xcor in coarse_classes:
        assert xleaf != xcor, '{:} vs. {:}'.format(xleaf, xcor)
    print("leaf level has {:} classes, coarse has {:} classes.".format(len(leaf_classes), len(coarse_classes)))
    cls_realcls = {}
    for cls in leaf_classes:
      cls_realcls[cls] = [cls]
    for cls in coarse_classes:
      cls_realcls[cls] = list(set(parser.get_descendants(cls)).intersection(set(leaf_classes)))
    all_keys = sorted( list(cls_realcls.keys()) )
    all_key_pairs = [ (acls, bcls) for acls in all_keys for bcls in all_keys ]
    
    print ('There are {:} keys and {:} pairs'.format(len(all_keys), len(all_key_pairs)))
    print ('{:} start calculating distance between each class pairs'.format(time_string()))
    cls_dis_info, cls_to_parent, cls_to_child = {}, {}, {}
    for idx, xpair in enumerate(all_key_pairs):
      cls_dis_info[xpair] = 1e9
    print ('There are {:} in cls_dis_info'.format( len(cls_dis_info) ))
    for idx, key in enumerate(all_keys):
      parents = set(parser.get_parent(key)).intersection(set(all_keys))
      childs  = set(parser.get_child (key)).intersection(set(all_keys))
      cls_to_parent[key] = list(parents).copy()
      cls_to_child [key] = list(childs).copy()
      for con in list(parents) + list(childs):
        cls_dis_info[(key,con)] = 1
        cls_dis_info[(con,key)] = 1
      cls_dis_info[(key,key)] = 0
    for idx, ccls in enumerate(all_keys):
      if idx % 100 == 0: print ('{:4d} / {:4d} ::: {:}'.format(idx, len(all_keys), time_string()))
      for acls in all_keys:
        for bcls in all_keys:
          if cls_dis_info[(acls,ccls)] < 1e9 and cls_dis_info[(ccls,bcls)] < 1e9 and cls_dis_info[(acls,bcls)] > cls_dis_info[(acls,ccls)] + cls_dis_info[(ccls,bcls)]:
            cls_dis_info[(acls,bcls)] = cls_dis_info[(acls,ccls)] + cls_dis_info[(ccls,bcls)]
    print ('There are {:} in cls_dis_info'.format( len(cls_dis_info) ))
    torch.save({'cls_realcls' : cls_realcls,
                'cls_dis_info': cls_dis_info,
                'cls_to_parent': cls_to_parent,
                'cls_to_child' : cls_to_child}, cache_file_name, pickle_protocol=4)
    print ('save cache class to leaf info into {:}'.format(cache_file_name))
  ...
  if not cache_file_name.exists():
    ...
    test_classes, train_classes, xiters = [], [], 0

    while True:
      if len(train_classes) / (train_ratio / (1-train_ratio)) <= len(test_classes): 
        add_to_train = True
      else:
        add_to_train = False

      if add_to_train: target_set = test_classes
      else           : target_set = train_classes

      ok_classes = []
      for xcls in all_graph_classes:
        if len(target_set) == 0: xdis = 1e9
        else: xdis = min([ cls_dis_info[(xcls, _x)] for _x in target_set ])
        if xdis >= min_dis_criterion:
      ...
      if add_to_train: train_classes.append( select_cls )
      else           : test_classes.append ( select_cls )
      xiters = xiters + 1
    for trcls in train_classes:
      for tecls in test_classes:
        assert trcls != tecls, '{:} vs. {:}'.format(trcls, tecls)
        assert cls_dis_info[(trcls, tecls)] >= min_dis_criterion, '{:} vs. {:}'.format(trcls, tecls)
