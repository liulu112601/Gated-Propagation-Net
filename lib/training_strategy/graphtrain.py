...
from models import euclidean_dist, update_acc, get_real_propagation_graph, get_max_spanning_tree_kruskal, get_adj

def graphtrain(lr_scheduler, mymodel, criterion, optimizer, logger, dataloader, hierarchy_info, epoch, args, mode, n_support, pp_running, n_hop):
  args = deepcopy(args)
  ...
  if mode == "train":
    mymodel.emb_model.train(); mymodel.propagation_net.train()
  ...
  for batch_idx, (_, imgs, labels) in enumerate(dataloader):
    if mode == "train" and len(imgs) == args.batch_size and len(set(labels.tolist())) != args.classes_per_it_tr:
      ...
      with torch.no_grad():
        ...
        # get nodes
        label2metalabel, forward_adj, backward_adj = get_real_propagation_graph(target_classes, hierarchy_info, args.n_hop, 5, 0.5) # if the num of children or parents > 5, only select half of them    
        ...
        for cls in target_classes:
          ...
          base_proto[ label2metalabel[cls] ] = torch.mean( embs[s_idx], dim=0)
      ...
      if args.coef_base == -1 and args.coef_prop == -1:
        if epoch == -1:
          ...
          final_proto = target_base_proto * (1-epoch/args.epochs) + target_prop_proto * epoch/args.epochs
      else:
      ...
      else: raise ValueError("undefined mode : {}".format(mode))
